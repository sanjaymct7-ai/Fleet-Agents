...
Phases: reset -> plan -> dispatch -> simulate -> track -> recover -> arbitrate/execute -> comms."""
...
def run_day(log=print):
    log("1/8 Resetting the world…")
    reset()

    log("2/8 Planning routes (OR-Tools)…")
    plan_and_save()

    log("3/8 Dispatching drivers…")
    dispatch()

    log("4/8 Simulating the day (compressed)…")
    simulator.SPEED = 0.01   # ~6s per sim-hour: fast, map still animates
    simulator.run()

    log("5/8 Tracking: raising exceptions…")
    scan(get_client())

    log("6/8 Recovery agent judging exceptions…")
    recover()

    log("7/8 Manager reviewing T2s, executor applying approved actions…")
    review()
    execute()

    log("8/8 Comms agent writing customer messages…")
    write_comms()
